...
{
if (acl_warn_headers != NULL)
  {
  DEBUG(D_receive|D_acl) debug_printf(">>Headers added by %s ACL:\n", acl_name);
  header_last->next = acl_warn_headers;
  acl_warn_headers = NULL;
  while (header_last->next != NULL)
    {
    header_last = header_last->next;
...
header, pointed to by the global anchor variable. Temporarily mark it as "old",
i.e. not to be used. We keep header_last pointing to the end of the chain to
make adding headers simple. */
  ...
  if (sender_address[0] == 0)
    ...
    if (sender_local || local_error_message)
      ...
      header_add(htype_from, "%sFrom: %s@%s\n", resent_prefix,
        authenticated_id,
        (submission_domain == NULL)? qualify_domain_sender : submission_domain);
...
if (from_header != NULL &&
  ...
  if (submission_mode)
    generated_sender_address = string_sprintf("%s@%s", authenticated_id,
      (submission_domain == NULL)? qualify_domain_sender : submission_domain);
  else
    generated_sender_address = string_sprintf("%s@%s", originator_login,
      qualify_domain_sender);
...
/* Data file successfully written. If an ACL from any RCPT commands set up any
warning headers to add, do so now, before running the DATA ACL. */

add_acl_headers(US"MAIL or RCPT");

/* Generate text for the Received: header by expanding the configured string,
and adding a timestamp. By leaving this operation till now, we ensure that the
timestamp is the time that message reception was completed. However, this is
deliberately done before calling the data ACL and local_scan().
...
if (received[0] == 0)
  ...
  header_list->type = htype_old;
  received_length = 10;                  /* length before timestamp */
  }
else
  {
  header_list->text = string_sprintf("%s; %s\n", received, timestamp);
  header_list->type = htype_received;
  received_length = Ustrlen(received);   /* length before timestamp */
  }
...
/* Update the timstamp in the Received: header to account for any time taken by
