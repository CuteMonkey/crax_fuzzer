  ...
  /* Otherwise we expect a word or a string. */
    ...
    /* Personal can be followed by any number of aliases */
      ...
      for (;;)
        ...
        ptr = nextitem(ptr, buffer, sizeof(buffer), TRUE);
        if (*error_pointer != NULL) break;
        aa = store_get(sizeof(alias));
        aa->name = string_copy(buffer);
        aa->next = c->left.a;
        c->left.a = aa;
        }
      }
...
static BOOL
test_header(uschar *s, BOOL cond, alias *aliases, int count, ...)
{
header_line *h;
int slen = Ustrlen(s);
...
switch (c->type)
  ...
  if (system_filtering) yield = FALSE; else
    {
    alias *aliases = c->left.a;
    uschar *self, *self_from, *self_to;
    uschar *psself = NULL;
    uschar *psself_from = NULL, *psself_to = NULL;
    int to_count = 2;
    int from_count = 5;
    ...
    yield = test_header(US"to:", TRUE, aliases, to_count, self, self_to,
              psself, psself_to) &&
            test_header(US"from:", FALSE, aliases, from_count, "server@",
              "daemon@", "root@", self, self_from, psself, psself_from) &&
            test_header(US"subject:", FALSE, NULL, 1, "circular") &&
            test_header(US"auto-submitted:", FALSE, NULL, 1, "auto-") &&
            test_header(US"precedence:", FALSE, NULL, 1, "bulk") &&
            test_header(US"precedence:", FALSE, NULL, 1, "list") &&
            test_header(US"precedence:", FALSE, NULL, 1, "junk") &&
...
while (commands != NULL)
  ...
  switch(commands->command)
    ...
    *error_pointer = fmsg;
    ...
    . If i is equal to mailarg_index_text its a text string for the body, where
      anything goes.
