...
/* Loop for quota, quota_filecount, and quota_warn_threshold */
 ...
 *    if (THRESHOLD_CHECK)
 *      saved_size = maildir_compute_size(check_path, &dircount, &old_latest,
...
*/
  ...
  /* Otherwise (saved_size is not yet set), if we are going to do a quota check
  later on, find the current size of the mailbox. (We don't need to check
  ob->quota_filecount_value, because it can only be set if ob->quota_value is
  set.) */

  if (saved_size < 0 && (ob->quota_value > 0 || THRESHOLD_CHECK))
    {
    DEBUG(D_transport)
...
if (ob->quota_value > 0)
  ...
  DEBUG(D_transport)
    ...
    debug_printf("  file count quota = %d count = %d\n",
      ob->quota_filecount_value, dircount);
    }
  if (saved_size + (ob->quota_is_inclusive? message_size : 0) > ob->quota_value)
    {
    ...
    errno = ERRNO_EXIMQUOTA;
           ...
           dircount + (ob->quota_is_inclusive ? 1:0) >
...
if (THRESHOLD_CHECK)
  ...
  DEBUG(D_transport)
    debug_printf("quota = %d threshold = %d old size = %d message size = %d\n",
      ob->quota_value, threshold, saved_size, message_size);
  ...
  *
  * if (ob->quota_value > 0 && threshold > 0 && saved_size > 0 &&
  *     saved_size <= threshold && saved_size + message_size > threshold)
  *
