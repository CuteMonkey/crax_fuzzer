...
DEBUG(D_transport|D_retry)
  debug_printf("added retry item for %s: errno=%d %d flags=%d\n",
    rti->key, rti->basic_errno, rti->more_errno, flags);
}
...
domain:IP-address. It doesn't seem necessary to go to that level of detail in
...
domain name, which is always just a domain.
...
/* If the key contains an @ we are probably handling a local delivery and
have a complete address to search for; this happens when retry_use_local_part
is set on a router. Otherwise, the key is likely to be a host name for a
remote delivery, or a domain name for a local delivery. We prepend *@ on the
front of it so that it will match a retry item whose address item pattern is
independent of the local part. The alternate key, if set, is always just a
domain, so we treat it likewise. */
