...
Setup the 'B<SERVER.TAB>' configuration option (after reading the rest of this
document well).
...
Setup the file 'B<smtprelay.tab>' if you want to extend mail relaying to IPs out
...
Copy XMail executables to 'B<C:\MailRoot\bin>'.
...
that point to the locally handled account 'B<homerE<64>internal-domain.org>'.
...
'B<authtype>' = authentication method ('B<CLR>' = USER/PASS auth,  'B<APOP>' = APOP auth).
...
It connect with the 'xmailserver.org' POP3 server and download all messages for 'dlibenziE<64>xmailserver.org' into
...
If the lookup succeed the 'B<SERVER.TAB>' variable 'B<DefaultSmtpPerms>' is used to assign
...
The section L<"SMTP Client Authentication"> explains the client part of custom
authentication when we put an 'B<external>' line inside the configuration file.
The file 'B<SMTPEXTAUTH.TAB>' is the server part of the custom authentication which has
the given format:
...
client response to server challenge (E<64>CHALL)
...
The external program must test all lines of 'B<@@FSECRT>' to find the one (if it exists)
that matches the client digest (B<@@DGEST>).
If it finds a match, it must return zero and overwrite 'B<@@FSECRT>' with the matching secret
(username + ':' + password).
If a match is not found, the program must return a value other than zero.
...
It also contains a file named 'B<USER.TAB>' that stores"account" variabiles, example:
...
'B<REMEMBER THAT THIS HOLDS ADMIN ACCOUNTS, SO PLEASE CHOOSE COMPLEX USERNAMES AND PASSWORDS AND
...
example of such setup is:

 "212.131.173.0/24"  "code=0"

In this case a code=0 tells XMail to flag the connection as possible spammer, but wait later
...
Be carefull if using @@FILE to give the external command enough timeout to complete,
...
Where  'B<auth-name>' can be any symbolic name and 'B<arg-or-macro>' can be a program
argument or one of these macros:
...
Then XMail decodes 'B<base64-challenge>' and invokes the external program to get the response
...
A comma separated list of SMTP servers XMail 'B<must>' use to send its mails.
...
=item [SmtpConfig-XXX.YYY.ZZZ.WWW]
...
rejection code ('B<6, 5 and 4>')and 'B<NEVER>' in case of passthru code ('B<7>') or modify code.
...
and these are 'B<FILTERS.PRE-DATA.TAB>' and 'B<FILTERS.POST-DATA.TAB>'. The file
'B<FILTERS.PRE-DATA.TAB>' contains one or more commands to be executed after the remote
...
response to the DATA command (final messages ack). The files 'B<FILTERS.PRE-DATA.TAB>'
and 'B<FILTERS.POST-DATA.TAB>' conatins zero or more lines with the following format:
...
if the command is listed inside 'B<FILTERS.PRE-DATA.TAB>' or inside 'B<FILTERS.POST-DATA.TAB>'.
Commands listed inside 'B<FILTERS.PRE-DATA.TAB>' will receive the following data stored
inside @@FILE:
...
defines all subdomains of 'B<xmailserver.org>' as alises of 'B<xmailserver.org>'.
...
authentication method ('B<CLR>' = USER/PASS auth 'B<APOP>' = APOP auth).
...
in 'B<SERVER.TAB>' (see L<"SERVER.TAB VARIABLES"> above) to use smart SMTP hosts.
