/* $Id: sycls.c,v 1.62 2004/06/20 21:48:05 gerhard Exp $ */
...
int Ftruncate(int fd, off_t length) {
   int retval, _errno;
   Debug2("ftruncate(%d, %d)", fd, length);
...
int Ftruncate64(int fd, off_t length) {
   int retval, _errno;
   Debug2("ftruncate64(%d, %d)", fd, length);
...
int Ioctl(int d, int request, void *argp) {
   ...
   if (argp > (void *)0x10000) {	/* fuzzy...*/
      ...
      Debug3("ioctl(%d, 0x%x, 0x%x)", d, request, argp);
   }
...
int Close(int fd) {
   ...
   _errno = errno;
   if (retval != 0)
      Warn2("close(%d): %s", fd, strerror(errno));
...
pid_t Waitpid(pid_t pid, int *status, int options) {
   int _errno;
   pid_t retval;
   Debug3("waitpid(%d, %p, %d)", pid, status, options);
   retval = waitpid(pid, status, options);
   _errno = errno;
   Debug2("waitpid(, {%d}, ) -> %d", *status, retval);
   errno = _errno;
   return retval;
}
...
int Bind(int sockfd, struct sockaddr *my_addr, int addrlen) {
   ...
   sockaddr_info(my_addr, infobuff, sizeof(infobuff));
...
int Accept(int s, struct sockaddr *addr, socklen_t *addrlen) {
   ...
   if (result >= 0) {
      char infobuff[256];
      sockaddr_info(addr, infobuff, sizeof(infobuff));
      Info5("accept(%d, {%d, %s}, "F_Zd") -> %d", s,
	     addr->sa_family, sockaddr_info(addr, infobuff, sizeof(infobuff)),
	     *addrlen, result);
   } else {
...
int Getsockname(int s, struct sockaddr *name, socklen_t *namelen) {
   ...
   Debug3("getsockname(, {%s}, {"F_socklen"}) -> %d",
	  sockaddr_info(name, infobuff, sizeof(infobuff)), *namelen, result);
   errno = _errno;
   return result;
}
#endif /* WITH_SOCKET */
...
int Getpeername(int s, struct sockaddr *name, socklen_t *namelen) {
   ...
   _errno = errno;
   sockaddr_info(name, infobuff, sizeof(infobuff));
...
int Setsockopt(int s, int level, int optname, const void *optval, int optlen) {
   ...
   _errno = errno;
   Debug3("setsockopt() -> (,,, 0x%08x, %d), %d",
       *(unsigned int *)optval, optlen, result);
   errno = _errno;
   return result;
}
#endif /* WITH_SOCKET */
...
int Recvfrom(int s, void *buf, size_t len, int flags, struct sockaddr *from,
   ...
   if (from) {
	  ...
	  from->sa_family, sockaddr_info(from, infobuff, sizeof(infobuff)),
	  *fromlen, retval);
   } else {
...
int Sendto(int s, const void *mesg, size_t len, int flags,
   ...
   int retval, _errno;
   Debug7("sendto(%d, %p[%08x...], "F_Zu", %d, %p, %d)",
	  s, mesg, htonl(*(unsigned long *)mesg), len, flags, to, tolen);
...
#if (WITH_IP4 || WITH_IP6) && HAVE_GETADDRINFO
...
#endif /* (WITH_IP4 || WITH_IP6) && HAVE_GETADDRINFO */
