/* $Id: xioopts.c,v 1.66 2004/06/20 21:18:17 gerhard Exp $ */
   ...
   *opts = Malloc((i+8)*sizeof(struct opt));
   ...
   while (optr == ',' || optr == '=') {
	 ...
	 Info2("setting option \"%s\" to %u", ent->desc->defname,
	       (*opts)[i].value.u_long);
	 break;
#if HAVE_TYPE_LONGLONG && HAVE_STRTOLL
      case TYPE_LONGLONG:
...
#  endif
	 ...
	 break;
#endif /* HAVE_TYPE_LONGLONG && HAVE_STRTOLL */
      case TYPE_UIDT:
	 if (a2 == NULL) { Error1("option \"%s\": value required", a0);
	    continue; }
	 if (isdigit((*a2)&0xff)) {
...
int applyopts_offset(xiofile_t *xfd, struct opt *opts) {
   struct opt *opt;
   unsigned char *ptr;
...
#if !NDEBUG
      default:
	 Error1("applyopts_single(): optioncode %d not implemented",
	       opt->desc->optcode);
#endif
      }
#endif /* 0 */
