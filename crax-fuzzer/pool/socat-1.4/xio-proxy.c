/* $Id: xio-proxy.c,v 1.16 2004/06/20 21:27:45 gerhard Exp $ */
...
static int xioopen_proxy_connect(char *a1, int rw, xiofile_t *fd,
...
const struct addrdesc addr_proxy_connect = { "proxy", 3, xioopen_proxy_connect, GROUP_FD|GROUP_SOCKET|GROUP_SOCK_IP4|GROUP_IP_TCP|GROUP_HTTP|GROUP_CHILD|GROUP_RETRY, 0, 0, NULL HELP(":<proxy-server>:<host>:<port>") };
...
*/
static ssize_t
   xioproxy_recvbytes(struct single *xfd, char *buff, size_t buflen) {
   ssize_t result;
   do {
...
static int xioopen_proxy_connect(char *a1, int rw, xiofile_t *xxfd,
				 unsigned groups, int dummy1, int dummy2,
				 void *dummyp1) {
   ...
   struct sockaddr_in them_sa, *them = &them_sa;
   char *proxyname, *proxyport = NULL;
   char *targetname;
   ...
   a2 = strchr(a1, ':');
   if (!a2) {
      Error1("xioopen_proxy_connect(\"%s\",,): missing target address", a1);
      return STAT_NORETRY;
   }
   *a2 = '\0', ++a2;
   targetname = a2;
   ...
   if ((a3 = strchr(a2, ':')) == NULL) {
      Error2("xioopen_proxy_connect(\"%s:%s\",,): missing target port", proxyname, a2);
      return STAT_NORETRY;
   }
   *a3 = '\0', ++a3;
   ...
   result = _xioopen_proxy_prepare(proxyvars, opts, targetname, a3);
   if (result != STAT_OK)  return result;

   result = _xioopen_ip4app_prepare(opts, &opts0, proxyname, proxyport,
				    protname, them, us,
...
#undef LEN
   ...
   proxyvars->targetport = htons(parseport(targetport, "tcp"));
...
#     define HEADLEN 256
   ...
   *strstr(request, " HTTP") = '\0';
   ...
   do {
      sresult = xioproxy_recvbytes(xfd, buff+offset, 1);
      if (sresult <= 0) {
