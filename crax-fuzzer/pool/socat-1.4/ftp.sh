#! /bin/sh
# $Id: ftp.sh,v 1.9 2004/06/06 17:33:22 gerhard Exp $
...
while :; do
    case "$1" in
    -socks) shift; method="socks4:$1" ;;
    -proxy) shift; method="proxy:$1" ;;
    -user) shift; user="$1" ;;
...
socat $SO1 fifo:$TO,nonblock,ignoreeof!!fifo:$FROM $method:$server:21 &
S1=$!
...
ftp_chat () {
    ...
    while read status message <&3;
       ( case "$status" in [0-9][0-9][0-9]-*) exit 0;; [0-9][0-9][0-9]*) exit 1;; *) exit 0;; esac )
    do :; done
...
echo $info |tr ',' ' ' |(read i1 i2 i3 i4 p1 p2 
    ...
    socat -u $SO2 $method:$server:$port -
) &
S2=$!
