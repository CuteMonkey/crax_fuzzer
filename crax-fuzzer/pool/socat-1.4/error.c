/* $Id: error.c,v 1.21 2004/06/06 17:33:22 gerhard Exp $ */
...
struct diag_opts diagopts =
  { NULL, E_ERROR, E_ERROR, 1, 0, NULL, LOG_DAEMON, false } ;
...
void diag_set(char what, const char *arg) {
   ...
   case 'D': diagopts.msglevel = strtoul(arg, NULL, 10); break;
   case 'e': diagopts.exitlevel = strtoul(arg, NULL, 10); break;
   case 'u': diagopts.micros = true; break;
   default: msg(E_ERROR, "unknown log device specification %c%s", what, arg);
   }
}
...
#define BUFLEN 256
   ...
   if (result < 0) {
	 ...
	 bytes += sprintf(buff+19, ".%06lu ", now.tv_usec);
      } else {
	 bytes =
...
#endif /* !HAVE_GETTIMEOFDAY */
   sprintf(buff+bytes, "%s["F_pid"] ", diagopts.progname, getpid());
   syslp = strchr(buff+23, '\0');
   sprintf(syslp, "%c ", "DINWEF"[level]);
   bufp = strchr(syslp, '\0');
   vsnprintf(bufp, BUFLEN-35-1, format, ap);
   strcat(bufp, "\n");
